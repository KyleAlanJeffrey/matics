...
A project is a folder whose name ends in `.matics`; folders without the extension are not
projects. macOS shows it as a document package (UTI `ai.stout.matics.project`, conforming
...
refused when it is opened, with a message naming what is missing. `imageUrl` and `Document.file` reference assets by a path
relative to the folder (`assets/...`). Inline data URL pictures (the browser build keeps
pictures that way) are written to `assets/` on first save.
...
Compressed projects: a `.matics` file (UTI `ai.stout.matics.compressed`) is a zip of a
project folder, `project.json` at the root and the flat `assets/` files beside it. Opening
one unpacks it into a new folder under the projects root, ignores any other entries and
always gives the project a new id, so it can be opened next to the project it came from.
There is no other project import or export format.
